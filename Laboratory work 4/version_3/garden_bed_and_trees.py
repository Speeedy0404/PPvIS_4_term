...
class GardenBedAndTrees:
    ...
    def show_information_about_objects(param: int, gui=False, root=None, list=None, label=None) -> None:
        if param == 0:
            ...
            if gui:
                it = len(list)
                list.append(label(text="---------------------------------------"))
                root.fruits.add_widget(list[it])
                it += 1
    ...
    @staticmethod
    def changing_the_water_from_the_weather(weather, param: int, show_next_day_view=ViewPrint.show_next_day_view, gui=False) -> None:
    ...
    @staticmethod
    def changing(number: int, param: int, show_next_day_view=ViewPrint.show_next_day_view, gui=False) -> None:
        if param == 0:
            if gui:
                way = 'vegetables'
        ...
        if param == 0:
            if gui:
                ...
                for i in range(0, count):
                    ...
                    root.inffo.add_widget(list_objects[it])
                    it += 1
                    list_objects.append(label(text="Уровень воды: {}".format(
