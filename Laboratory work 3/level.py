import pygame
from support import import_scv_layout, import_cut_graphics
...
class Level:
    def __init__(self, current_level, surface, create_overworld, change_coins, change_health, level_passed,
                 change_score):
        # general setup
        ...
        # player setup
        player_layout = import_scv_layout(level_data['player'])
        ...
        # terrain setup
        terrain_layout = import_scv_layout(level_data['terrain'])
        self.terrain_sprites = self.create_tile_group(terrain_layout, 'terrain')

        # crate setup
        crate_layout = import_scv_layout(level_data['crates'])
        self.crate_sprites = self.create_tile_group(crate_layout, 'crates')

        # grass setup
        grass_layout = import_scv_layout(level_data['grass'])
        self.grass_sprites = self.create_tile_group(grass_layout, 'grass')

        # coins setup
        coin_layout = import_scv_layout(level_data['coins'])
        self.coin_sprites = self.create_tile_group(coin_layout, 'coins')

        # foreground palms setup
        fg_palm_layout = import_scv_layout(level_data['fg palms'])
        self.fg_palm_sprites = self.create_tile_group(fg_palm_layout, 'fg palms')

        # background palms setup
        bg_palm_layout = import_scv_layout(level_data['bg palms'])
        self.bg_palm_sprites = self.create_tile_group(bg_palm_layout, 'bg palms')

        # enemy setup
        enemy_layout = import_scv_layout(level_data['enemies'])
        self.enemy_sprites = self.create_tile_group(enemy_layout, 'enemies')

        # constraint setup
        constraint_layout = import_scv_layout(level_data['constraints'])
    ...
    def horizontal_movement_collision(self):
        ...
        collectable_sprites = self.terrain_sprites.sprites() + self.crate_sprites.sprites() + self.fg_palm_sprites.sprites()

        for sprite in collectable_sprites:
    ...
    def check_death(self):
        if self.player.sprite.rect.top > screen_height:
            self.player.sprite.change_health(-100)

    def check_win(self):
        if pygame.sprite.spritecollide(self.player.sprite, self.goal, False):
            self.level_passed(self.current_level)
